...
integrated contract-only history at signed commit
...
D-130 freezes the separate G2-7F reacceptance contract at
[`global_v2_maplight_robustness_scoring_capability_reacceptance_contract.json`](../../benchmarks/openadmet_cyp_2026/global_v2_maplight_robustness_scoring_capability_reacceptance_contract.json),
SHA-256 `b414b0cf...efae698`. It changes no scoring science and reuses the exact
D-129 compiler only while its SHA-256 remains `6f15205f...6f492`. The new path
has one distinct attempt ID, no root/output CLI override, and fixed root
`/tmp/cypshift-g2-7f/scoring-capability-attempt-1`. Before creating even the
fixed parent, a future V2 driver must authenticate every parent and source,
require both roots and terminals absent, prove the path resolves exactly with
at least four components, and invoke the accepted cleanup predicate on the
still-absent root. It must remove root and empty parent before publishing one
...
4. read D-082 through D-143 in `docs/strategy/DECISIONS.md`;
...
the next implementation replaces the historical live official driver and
five-field claim derivation. Its contract SHA-256 is
`6703ad30...45709c2c`, and six static
...
D-141 may retire exactly three historical nodes: the D-135 live-implementation
...
the D-141 hook and paired with a replacement current-state assertion in the
new D-141 focused tests; all other historical audits remain active. This is a
...
tests. The safe repository suite, run with permanently barred
`tests/test_openadmet_global_v2_maplight_robustness_synthetic.py` explicitly
ignored, then reported `1415 passed, 8 skipped, 2 failed` in about 352.96
seconds.
Both failures are source-shape assertions in immutable D-134 focused snapshot
SHA-256 `3fedd87eb86f485167a53564cb440409056d82982f329db888028e294228c53f`:
...
D-140 therefore freezes only the supplemental source-shape transition contract
...
pass.
The exact D-141 replacements are
`test_corrected_child_preserves_fit_topology_and_cleans_before_terminal_staging`,
covering Stage A/B/C identities `540/180/300`, five exact feature widths,
predictor-authority cross rejection, stage order with exact condition
`selected != "G2-7-M0-FULL"`, and cleanup-before-staging; and
`test_supervisor_precedes_claim_consumption_and_common_seal_owns_terminal_publication`,
covering supervisor/claim/child chronology, exact `raw_observed`, both
publication-root/parent arguments, equal limits, fixed-root absence,
aggregate-bytes-only `_failure_payload` accounting, common-seal-exclusive
publication, and absence of the obsolete child/parent terminal names. D-141
may add only these two skip markers while preserving the prior four, for six
total, and must bind
`d140_source_shape_transition_contract_sha256` through every frozen location.
...
all three Python lanes. A fresh read-only official preflight then authenticated
the frozen contract/receipt lineage, public tracked template, fixed-root
absence, runtimes, supervisor, resource headroom, and metadata-only source and
baseline allowlists. The sole fixed no-argument G2-7G official driver ran once
under cumulative supervision, consumed the private claim, exited zero, and
published clean terminal `G2_7_MAPLIGHT_ROBUSTNESS_UNDERPOWERED`.

D-143 packages only its privacy-safe aggregate projection at
...
and size 9,945 bytes, plus the standalone public-only audit
...
size 21,081 bytes, at `1/1 passed`. The public
record is duplicate-key/nonfinite rejecting, sorted two-space canonical JSON
with one terminal LF. It projects only fixed lineage, private aggregate hashes,
allowed accounting, bounded cumulative resources, cleanup, the contracted
support-stop summary, and denied authority. It does not copy the private
receipt, manifest, or preflight, and publishes no absolute/private path,
support table, molecule, structure, target, prediction, feature matrix,
component membership, model, process identifier, unrestricted log, or portal
observation.

This terminal is authoritative official execution evidence, but not scientific
model-quality evidence. After the private claim was consumed, the trusted
compiler opened exactly 19,620 direct endpoint rows, 73,575 group-fold rows,
5,197 finite central point values, 4,905 feature-identity rows, 19,620
feature-matrix rows, and 24,525 total feature rows. It opened no official
baseline prediction, scoring truth, or training-target capability. Stage A,
Stage B, and Stage C each performed zero fits and generated zero predictions;
the tutorial metric and development metric were never called. The manifest
therefore has `selected_candidate=null`, `selection_tokens=0`, and
`runner_ups=0`. No row-level value or model binary was retained.

Every frozen numeric minimum passed across all 240 active support cells. The
one exercise-oracle failure has exact reason
`TAUTOMER_MERGED:confirmatory_touch_not_exercised`. The frozen predicate
requires confirmatory-touch exercise before fitting; the public projection
intentionally omits observed support and exclusion values. This is a
label-free support/preflight failure, not endpoint or model performance.

The completed local safe repository suite, with
`tests/test_openadmet_global_v2_maplight_robustness_synthetic.py` explicitly
ignored as permanently barred, reports `1425 passed, 10 skipped, 2 failed` in
357.51 seconds. The only failures are
...
Both retain historical pre-execution absence premises that became false when
the authorized one-use root was immutably sealed. This is expected negative
state-transition evidence. It does not contradict the D-143 public record,
underpowered terminal, privacy boundary, mechanics, or scientific
interpretation.

The exact accepted 13-field cumulative observation has integer-zero return,
three acknowledged checkpoints, two observed descendants, complete cleanup,
isolated network, hidden GPU environment, and zero GPU-hours, detached
children, and warnings. It used 33.09143570300148 wall-seconds,
33.194318974000005 CPU-seconds, 8,192 peak storage bytes, and 289,660,928 peak
simultaneous RSS bytes, all below their hard maxima. Cleanup removed the
restricted work root and claim/publication staging before the shared bounded
seal. The fixed private attempt root intentionally remains as the immutable
two-entry root `attempt_claim.json` plus `terminal`; its terminal contains
exactly `attempt_receipt.json`, `manifest.json`, and `preflight.json`. The
tracked public claim template remains byte-identical, unusable, and
unconsumed; the sole private derivation is permanently consumed.

The D-122 full-default rule cannot be applied after this outcome. It belongs to
the completed Stage-A candidate-selection ordering and would produce one
selection token. UNDERPOWERED is a distinct pre-fit status that requires no
selected candidate and zero token. Thus fixed MapLight remains the best
previously validated internal system at component-macro MAE `0.5837812652`,
but it was neither reselected nor robustness-validated by G2-7G. The frozen
G2-8 boundary requires an accepted G2-7 contender lock; D-143 creates none and
grants zero confirmatory, blinded-test, TDI, full-training, submission,
validator, official-metric, leaderboard, portal, credential, upload, science,
or model-quality authority.

D-144 is integrated as signed commit
`5d7ed5db76ec0928ba34e19e72ab839ee556d51e`; PR #181 CI run `33123874692`
and exact-SHA post-main CI run `33124525495` are green. It freezes the
post-attempt test-transition contract
`benchmarks/openadmet_cyp_2026/global_v2_maplight_robustness_post_attempt_test_transition_contract.json`
at SHA-256 `d5eb773fc2584deaf31c5f3a3a283e365b6540d0c714fd08cb70ec02937b735f`
...
at SHA-256 `a654075771d9f42ac3a7dcbf058e8ca4dba879660888c2aa4262d1e4ea60a1fa`
(29,788 bytes), with `1/1 passed`. The contract binds four node markers,
preserves the prior six, requires ten after transition, and limits
D-145 to `tests/conftest.py` plus one comprehensive public audit of exact
collection/hash state and all surviving root-node semantics. Using public
static evidence only, that audit must preserve D-133 historical
...
and exact-SHA post-main CI run `33126546606` are green. It changes only
`tests/conftest.py` at SHA-256
...
at SHA-256 `2a58d9423aa99f6b441b9d173b9e2c9e263117ced46343e7e90acf90bad7eac3`.
The result is exactly ten markers and one active comprehensive audit. Focused
tests passed `2/2`; the bounded safe suite, with the barred G2-7B test
explicitly ignored, completed with `1425 passed, 14 skipped, 0 failed` in
341.76 seconds. Historical tests and D-143 evidence remain unchanged; the
audit imports no driver and inspects no protected state. D-145/D-146 open zero
...
D-148 is integrated as immutable signed commit
`f0f3b6f9380eebef0b03d87f29eb659ffc84f8d5` with green PR #185 CI run
`33337794223` and exact-SHA post-main CI run `33338342415`. It freezes the
...
D-148's never-activated formal capability prospectively required three isolated
eight-row paths over the exact bare 57-key GIN, accepting only identical tensor
and graph manifests and either exact embeddings or the predeclared absolute
`1e-7` numeric branch plus 48 exact fallback predictions. It also prospectively
required two uncached
...
fits, while retaining 20% prefit wall/CPU/storage/RSS margin and zero GPU. None
of that execution occurred.
...
official/private science. Its prospective later implementation/run schedule
never activated and is superseded by D-149's terminal rejection.

D-149 freezes status `G3_2_EXP_G4_GIN300_PRECLAIM_CLOSED` through
...
The attempted transition contract at
`2c11b90d08038a05efd01bd40cb92ac79bc74544cc67807d2dd9b09111fa94af`
(72,296 bytes / 582 LF), its static test at
`0c685112421929b715912450e8eeb0e8e7ae5534806c19a7bee8fac6ecdada2d`
(67,654 bytes / 1,905 LF), and the corrected contract at
`1def7c6c31a84508e8d50f67a817ff486c30fad3f502dbefcc094e7b6ea7615f`
(79,319 bytes / 646 LF) are non-integrable and confer no authority.

The terminal audit found that the attempted transition's broad zero-accounting
claims could not truthfully describe its public CI validation. The first safe
suite passed `1452 passed, 14 skipped, 0 failed` in 358.81 pytest seconds /
359.31 wall-seconds at 900,212 KiB maximum RSS, but its network/download,
import/model-operation, and cache-effect totals were not instrumented or
retained and remain unknown. An `UV_OFFLINE=1` isolated build produced a
338,214-byte sdist and 406,405-byte wheel with incidental non-frozen hashes;
one temporary CPython 3.12.3 environment installed cached `cypshift
0.2.0.dev0`, NumPy 2.5.2, Pillow 12.3.0, and RDKit 2026.3.5. No download was
observed, but raw build/install network transaction and byte totals remain
unknown.
...
access-time effects, and the first suite's corresponding totals were not
instrumented and remain unknown. One pre-audit failed closed on an already-
existing path. The preexisting ignored `.mypy_cache/3.12/cache.3.db` was
observed modified, but the exact mutating operation was not instrumented and
remains unknown; the authoritative mypy rerun used temporary state. Explicit
task temporary roots were deleted and are absent.
...
This public CI work is not G4 execution. The fixed G4 vector is comprehensive
zero: all seven restricted roots and seven future implementation paths stayed
absent; no isolated runtime/wheel, checkpoint/model body or tensor, scientific
import, parity process, graph, embedding, synthetic root, fit, prediction,
official/private/confirmatory/blinded-test/TDI read, metric, bootstrap,
selection token, contender, systemd unit, delegated cgroup, claim, result,
submission, validator, leaderboard-selection observation, portal credential,
upload, or GPU use occurred. `EXP-G4-GIN300` is permanently closed preclaim
with no retry, repair, replacement, alternate contract, or successor
correction. The D-149 terminal package changes exactly nine paths: the
rejection record, its strict test, these six narratives, and the experiment
ledger.
...
(2,919 bytes / 55 LF) and records the historical two-rehearsal 750-row /
3,000-finite-prediction candidate plus pinned-validator valid/zero-error result.
Current private candidate existence and bytes are deliberately unknown and
unopened; it is not currently reauthenticated or upload-ready.

Fixed MapLight remains the strongest validated internal baseline at component-
macro MAE `0.5837812652150708`, prior internal development evidence rather than
