...
Phase 2 Global-v2 is closed at its terminal scientific stop under D-084 through
D-146. The distinct Global-v3 `EXP-G4-GIN300` scientific contract is integrated
as signed D-147 commit `b5cf47c6bc8ccc2dc29c7167b1a436d792338509`;
PR #184 CI run `33327853790` and exact-SHA post-main CI run `33328374514` are
green. D-148 is immutable integrated contract-only history as signed commit
`f0f3b6f9380eebef0b03d87f29eb659ffc84f8d5`; PR #185 CI run `33337794223`
and exact-SHA post-main CI run `33338342415` are green. D-149 terminally closes
the genuinely distinct G4 lane before claim consumption at
`G3_2_EXP_G4_GIN300_PRECLAIM_CLOSED`. No implementation, capability run, G4
claim, or G4 scientific attempt follows. D-149 is integrated as signed commit
...
the conditional read-only reauthentication handoff for the historically
accepted direct MapLight candidate. The one-use result and any human-
authorized upload remain separate later milestones. This does not reinterpret
G2-7G; its immutable UNDERPOWERED terminal, zero selection, and closed G2-8
boundary remain unchanged. The
externally supplied 2026-08-24 audit has a privacy-sanitized public copy at
...
The closed Global-v2 program tested a family-safe, cross-fitted heterogeneous
global ensemble followed by masked-multitask and provenance-controlled external
transfer ablations. Its default-to-global residual TRACE-v2 hypothesis was not
activated because no accepted stronger global successor produced the required
positive OOF evidence.
...
milestone. The next implementation will make exactly three D-136 audit nodes
obsolete by design:
...
implementation change itself. It freezes the exact three-to-three transition
for the next implementation, now D-141 after D-140's supplemental contract:
mark only those node IDs, retain every other historical audit, and bind each
retirement to replacement current-state assertions in the new focused suite.
No implementation, historical audit, hook, science, official, claim, fit,
prediction, metric, confirmatory, blinded-test, submission, leaderboard, or
upload operation occurs in D-139. Both fixed one-use gates remain unrun and
...
`185555b2...a0df90a0` freeze that mapping and zero authority.
...
D-140 now freezes the supplemental contract-only source-shape transition in
[`global_v2_maplight_robustness_official_orchestration_source_shape_transition_contract.json`](../../benchmarks/openadmet_cyp_2026/global_v2_maplight_robustness_official_orchestration_source_shape_transition_contract.json),
SHA-256 `d4ff0e57b4c5d8b6bae808d0749f5b8e116965f18f2df3fee6e04e58dd727417`,
with seven contract tests at SHA-256
...
The two failures are negative pre-integration evidence, not an implementation
acceptance: they are exactly
...
in immutable D-134 snapshot SHA-256
`3fedd87eb86f485167a53564cb440409056d82982f329db888028e294228c53f`.

D-140 freezes exactly two D-141 replacements. The first maps to
...
it must cover Stage A/B/conditional-C fit identities `540/180/300`, exact five
candidate widths `2563/1539/1539/2248/2363`, both predictor-authority cross
rejections, Stage A -> selection -> Stage B -> conditional Stage C chronology
with exact condition `selected != "G2-7-M0-FULL"`, and `_terminal_bytes ->
...
it must prove outer supervision before claim consumption, exact child claim and
Stage A chronology, exact `raw_observed = supervisor.run_supervised`, both
`publication_root=PUBLICATION_STAGING_ROOT` and
`writable_publication_parent=OFFICIAL_ATTEMPT_ROOT.parent`, equal limits, the
fixed absent official root, `_failure_payload` with complete accounting that
returns aggregate bytes only with no terminal path or publication,
common-seal-exclusive publication, and removal of child
`PENDING_TERMINAL_ROOT` and parent `_finalize_terminal`.
D-141 may add only these two skip markers while preserving the prior four, for
six total, and must bind field
`d140_source_shape_transition_contract_sha256` through the corrected driver,
composite driver and receipt, and focused tests.

D-140 changes no implementation, `tests/conftest.py`, historical snapshot,
science, claim, gate, candidate, fit, prediction, metric, resource, selection,
privacy, or one-use state. At that milestone the prospective D-141
implementation remained uncommitted and unbound; no implementation hash is
D-140 evidence. Signed D-140 commit
...
D-141 now packages only the corrected official driver, fixed no-argument
composite acceptance driver, dedicated focused tests, and exact six-marker
collection transition. Their exact SHA-256 values are, respectively,
...
and `03d92bf3a2890a61190a6a4fc7a6bc59fa900ed6ea4b904223b1f2f991699d95`.
The implementation preserves the frozen D-135 science kernel and D-122
battery while implementing D-137/D-138's five-status, cumulative-supervision,
authoritative-accounting, atomic-claim, bounded common-seal, inode/mode,
symlink-safe cleanup, and resource rules. It completes the exact D-139
three-node and D-140 two-node historical transitions and binds
`d139_test_transition_contract_sha256` and
`d140_source_shape_transition_contract_sha256` through the live composite and
attempt receipt lineages.

Validation is green: 103 focused orchestration tests; the immutable D-134 file
at `6 passed, 3 skipped`; the D-136 audit at `4 passed, 3 skipped`; and 128
current D-137-through-D-141 related tests. The exact safe repository command
`uv run --locked pytest --ignore=tests/test_openadmet_global_v2_maplight_robustness_synthetic.py`
reported `1425 passed, 10 skipped, 0 failed` in 346.49 seconds (346.98 wrapper
wall-seconds; 902,124 KiB maximum RSS). Diff integrity, Ruff across explicitly
allowed Python paths, strict mypy across 78 source files, package build, and
two-root installed-wheel determinism also pass. The earlier 118-test
prospective result is superseded by this exact current evidence.

D-141 is implementation and synthetic-test evidence only. It ran neither the
fixed composite acceptance nor official attempt. Disposable synthetic fixture
claim mechanics were exercised only on temporary public roots; no tracked
claim was mutated or consumed, no private claim was created or opened, and no
official or private byte was opened. D-141 grants no science, model-quality,
formal-acceptance, official, confirmatory, submission, or upload authority.
Fixed MapLight remains the best validated internal system at component-macro
MAE `0.5837812652`.
...
D-147 freezes only the new Global-v3 `EXP-G4-GIN300` contract and its public
static audit. The canonical contract is
`benchmarks/openadmet_cyp_2026/global_v3_g4_gin300_contract.json`, SHA-256
`b48dc0c39c12b06cdd99693539cca18b99c73d8b801e81a416e52a798df8fd4e`
(37,092 bytes / 485 lines). Its strict public audit is
`tests/test_openadmet_global_v3_g4_gin300_contract.py`, SHA-256
`c32e8054da4c92f763c065c8d58d340993f6917c5c6a6d3580659febea69e3dc`
(31,371 bytes / 815 lines), at `9/9 passed`. The hypothesis adds exactly one
frozen 300-column supervised-masking GIN block after the unchanged 2,563
MapLight columns while retaining the accepted fixed CatBoost learner. A future
development attempt, if separately authorized, compares the true GIN candidate
with fold-local whole-vector-shuffle and Gaussian-noise controls using exactly
180 new fits, 140,688 new outer-prediction rows, zero baseline refits, one
synchronized 2,000-replicate component-bootstrap stream, and the frozen
conjunctive promotion and attribution gates. Historical binary CYP2C9, CYP2D6,
and CYP3A4 GIN gains are hypothesis support only: they are not OpenADMET pIC50
evidence, include no historical CYP1A2 result, and cannot establish separation
from the unknown pretraining corpus.
...
and installed-wheel two-root reproduction are green. These checks are
repository and CI parity evidence only and create no model-quality or
downstream authority.

The current Space snapshot permits pretrained models, but D-147 treats rights,
license notices, exact SNAP/DGL-LifeSci/MolFeat object identity, tensor coverage,
Linux parity, and nonredistribution as mandatory prospective eligibility gates.
Unknown OpenADMET structure and assay overlap must be disclosed and forbids
clean-zero-shot, uncontaminated-external-validation, and pretraining-family-
separation claims. D-147 consists of one contract, one public static test, and
the exactly disclosed bounded public source-rights/hash audit. It changes no
dependency or runtime. During contract preparation, that audit downloaded one
SNAP source archive to temporary non-Git storage,
temporarily persisted 33 public checkpoint files totaling 204,567,885 bytes,
and opened one 7,452,448-byte SNAP checkpoint only for hashing. It downloaded
zero DGL or MolFeat checkpoint bytes, deserialized or executed zero checkpoint
tensors, and added no checkpoint to Git or the workspace. That bounded
source-rights/hash evidence grants no current checkpoint-fetch, model-load, or
execution authority. D-147 opens no pretraining row, official input, structure,
target, baseline prediction, or feature row and performs zero feature builds,
fits, predictions, metrics, claims, selection tokens, contender locks,
confirmatory/test/TDI operations, submission rows, validator calls, leaderboard
selection, portal access, credential access, uploads, or GPU work.

D-148 is immutable integrated history at signed commit
`f0f3b6f9380eebef0b03d87f29eb659ffc84f8d5`; PR #185 CI run `33337794223`
and exact-SHA post-main CI run `33338342415` are green. It freezes the canonical
capability contract
...
(184,100 bytes / 1,687 lines). Its bound Linux x86_64 CPU-runtime manifest has
SHA-256
`67b58fc5eb9d1d3c0652bad9fa85eb1e688ed4bfb93d9ee107cad4db3e0ace01`
(166,425 bytes / 4,008 lines), and its third-party notices have SHA-256
`b76b026a7ed61c0c33cc9f78d66ca235e01e9d2c504126238e0f6e0f58e18deb`
(12,456 bytes / 241 lines). The strict public capability-contract audit has
SHA-256
...
325 allowed Python paths with the exact barred G2-7B runner, driver, and test
...
two-root audit/train/predict/report replay are green. The two smoke roots are
byte-identical at 9 files / 36,758 bytes each. Package hashes and Git status are
unchanged. Incidental build and smoke-tree hashes remain validation-only and
are not frozen.

The runtime manifest binds install-only CPython 3.10.13, 96 CPU-only wheels,
185 dependency edges, 539,395,366 wheel bytes, and 97 distribution artifacts
totaling 566,804,656 bytes. It includes the complete
`molfeat[dgl,transformer]` closure and excludes CUDA, NCCL, and Triton. The sole
post-cutoff package-wheel exception is `future==1.0.0`; the frozen 20240224
interpreter distribution is separately disclosed infrastructure. Its never-
activated execution would have authenticated 98 runtime network files including the 65-byte
sidecar (566,804,721 bytes total), three model/checkpoint objects totaling
22,374,079 bytes, one 606-byte MolFeat metadata object, and two redirects.

The immutable D-148 formal capability oracle prospectively required three
isolated CPU processes and exact tensor/graph parity over the eight-row
redistributable fixture. All paths would have had to deserialize the bare
`dgllife.model.gnn.gin.GIN` with the same 57 unprefixed keys. Embeddings would
have had to be byte-identical or satisfy only the predeclared
`max_abs <= 1e-7`, `rtol=0` branch and then yield 48 exact fallback predictions
from the two candidate models. Resource projection prospectively required two uncached
3,908-row GIN passes, the 3,908-molecule / 3,640-component official-shaped
synthetic topology, 180 model-double fits and 140,688 predictions per root, six
real CatBoost fits with 4,692 validation predictions, 20% wall/CPU/storage/RSS
margin, and zero GPU.

D-148 is contract/public-evidence only. It created no runtime, wheel cache,
private object root, claim, or result; downloaded no wheel,
interpreter-archive, checkpoint, or model body; deserialized or executed no
checkpoint; ran no parity, GIN, CatBoost, feature, fit, or prediction work; and
opened no official/private scientific input. The audit recorded six
checkpoint/model HEAD requests with zero body bytes, a 606-byte MolFeat
metadata GET plus one HEAD, two interpreter archive/sidecar HEAD requests with
zero body on those HEADs, 95 distinct PyPI JSON resources, nine SPDX JSON resources,
the PyTorch CPU simple index, a 65-byte checksum-sidecar GET, and a 1,495-byte
build-license GET. Uninstrumented total public-source transactions and bytes
remain unknown rather than inferred.

At the D-148 freeze, its prospective schedule assigned implementation and one
formal capability run to later milestones. That schedule never activated and
is superseded by D-149's terminal preclaim rejection. D-149 freezes status
`G3_2_EXP_G4_GIN300_PRECLAIM_CLOSED` through
...
The transition contract at SHA-256
...
(67,654 bytes / 1,905 LF), and its corrected contract at SHA-256
`1def7c6c31a84508e8d50f67a817ff486c30fad3f502dbefcc094e7b6ea7615f`
(79,319 bytes / 646 LF) are all non-integrable and confer no authority. The
terminal audit found that their unqualified zero-accounting claims could not
truthfully describe the public CI validation already performed.

The first bounded safe suite passed `1452 passed, 14 skipped, 0 failed` in
358.81 pytest seconds / 359.31 wall-seconds at 900,212 KiB maximum RSS. Its raw
network/download, import/model-operation, and cache-effect totals were not
instrumented or retained and remain unknown. An `UV_OFFLINE=1` isolated build
produced one 338,214-byte sdist and one 406,405-byte wheel; their hashes are
incidental and non-frozen. One temporary CPython 3.12.3 environment installed
cached `cypshift 0.2.0.dev0`, NumPy 2.5.2, Pillow 12.3.0, and RDKit 2026.3.5.
No download was observed, but raw build/install network transactions and bytes
were not instrumented and remain unknown.

Two successful public-fixture audit/train/predict/report roots were byte-
identical at 9 files / 36,758 bytes each. Each reported 7 accepted,
1 quarantined, 7 warnings, 3 supported, 1 unsupported, and 21 predictions, for
2 train, 2 predict, 2 report, and 42 prediction operations. Internal library
fit/metric/import totals, build-environment/cache counts, global cache mutation/
access-time effects, and the first suite's corresponding totals remain
unknown. One pre-audit failed closed on an already-existing path. The
preexisting ignored `.mypy_cache/3.12/cache.3.db` was observed modified, but
the exact mutating operation was not instrumented and remains unknown; the
authoritative mypy rerun used temporary state. All explicit task temporary
roots were deleted and are absent.
...
These public CI operations are not G4 execution. The fixed G4 vector is
...
credential, upload, or GPU use occurred. `EXP-G4-GIN300` is permanently closed
preclaim with no retry, repair, replacement, alternate contract, or successor
correction. D-149's terminal package changes exactly nine paths: the rejection
record, its strict test, these six narratives, and the experiment ledger.
...
The strongest viable submission backout is independent of G4 and G2-8.
...
(2,919 bytes / 55 LF). It records 750 rows, 3,000 finite predictions, and a
historically valid pinned-validator result with zero errors. Current private
candidate existence and bytes are deliberately unknown and unopened, so the
candidate is not currently reauthenticated or upload-ready.

Fixed MapLight remains the strongest validated internal baseline at component-
macro MAE `0.5837812652150708`; it was neither selected nor
robustness-accepted by G2-7G. The MAE is prior internal development evidence,
not an official or leaderboard score, reselection, or robustness result. G2-8
stays closed. D-149 grants no submission regeneration, validator, leaderboard,
portal, credential, or upload authority.
...
green across Python 3.11, 3.12.3, and 3.14. The merge preserved that same
signed commit without rewriting.

D-150 freezes the separate direct-baseline reauthentication handoff contract
at
...
and status `DIRECT_BASELINE_REAUTHENTICATION_HANDOFF_CONTRACT_FROZEN`, SHA-256
`589facbbe8b51aeee00abdcba756c9119262572954854f438c549cab7ff98fcd`
(22,489 bytes / 431 LF), with strict public static test
...
`submission_contract.json`/`4be9933c...9a3c`. It requires 750 rows, the six
...
so neither D-150 nor later raw-byte reauthentication can claim a current
validator pass or upload readiness.

The contract binds the immutable handoff and historical submission/manifest
receipts against that latest-tracked public snapshot. It preserves the
historical 750-row, 3,000-finite-prediction, pinned-validator valid/zero-error
evidence without claiming a current result. Current private candidate
existence, bytes, and readability remain deliberately unknown and unopened;
the current validator result is unknown and uninvoked.
D-150 does not reselect MapLight, does not robustness-accept it, and does not
open G2-8.

D-150 changes exactly nine paths: the contract, its strict public test,
`benchmarks/openadmet_cyp_2026/README.md`,
`docs/phases/PHASE_2_OPENADMET_GLOBAL_V2.md`, `docs/phases/README.md`,
`docs/strategy/DECISIONS.md`, `docs/strategy/NEXT_ORCHESTRATOR_PROMPT.md`, this
file, and `runs/experiment_ledger.csv`. The milestone reads zero private
...
selection, upload, or GPU operation. It adds no runner, validator wrapper,
upload integration, model, or private artifact.

Only after D-150 receives independent review, final identity freeze, signed
fast-forward integration, and green exact-SHA post-main CI may a separate
one-use read-only reauthentication result inspect the fixed candidate and call
no validator. On success it opens and enumerates the root exactly once and
opens each exact file exactly once only to hash raw bytes in memory. On failure
each operation is attempted at most once, the first defect stops the protocol,
and no later operation runs. With no parsing, copying, retention, or official-
data access, it may authenticate only the unchanged historical candidate.
Missing or unreadable bytes, submission or manifest identity drift, latest-
tracked public snapshot identity drift, any need for parsing or a validator,
or any
accounting/cleanup defect stops the direct route without regeneration, refit,
reprediction, rewrite, reorder, reformat, repair, replacement, retry, or an
alternate candidate. Result publication is at most one no-replace write. A
safely classified success or failure publishes one result only after all
descriptors are closed and in-memory candidate bytes are discarded, at
...
invocation and withdraws the route without retry. G4 and G2-8 are already
closed, so that stop is the competition backout condition for this build. A
...
live upload remain a later candidate-specific human-authorized operation.
...
predictions. This immutable receipt proves the historically accepted direct
competition candidate; current private candidate existence and bytes remain
deliberately unknown and unopened pending separate reauthentication. Repository
evidence intentionally makes no claim about private portal activity or results,
and none may select a model. Its manual-upload handoff is recorded in
...
is historical evidence for an accepted direct baseline candidate, not current
private-byte reauthentication, TRACE evidence, or an official score. Private
portal activity and results are intentionally outside repository evidence and
cannot select a model, threshold, feature, or candidate.
...
Finalize D-150's exact nine-path contract/test/narrative/ledger package.
Recompute the final contract/test identities and sizes, require canonical
bytes, the focused static audit, exact nine-path scope, and the bounded safe
checks. Use one coherent SSH-signed `zchboswell` commit, exact-head pull-request
checks, local fast-forward-only integration, a non-rewriting push, and green
exact-SHA post-main CI. A package defect blocks D-150 without opening the
candidate.

Only after that gate may the separate one-use read-only reauthentication result
milestone open each fixed candidate file at most once only to hash raw bytes in
memory. It may not parse, validate, copy, retain, or open official data. Do not
combine it with regeneration, repair, portal access, or upload. A failed gate
backs this build out of the competition path; a clean gate produces only a
human handoff, and any live upload still requires a later candidate-specific
human authorization.
