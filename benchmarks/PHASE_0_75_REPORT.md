...
The jointly frozen public families also reproduce the dated MapLight anchors,
as described below. They do not establish clean external validation or
challenge transfer.

## Bounded public comparator reproduction

Two label-free public prediction attempts independently generated byte-identical
payloads for both families before any Phase 0.75 public label was opened. Their
manifests are `014ebbea...` and `e8e65e7d...`. A separate reviewed scorer then
performed exactly 36 AUPRC calls: five seed columns and one separately labeled
arithmetic mean-probability column for each family and task.

| Family | CYP2C9 mean ± SD | CYP2D6 mean ± SD | CYP3A4 mean ± SD |
| --- | ---: | ---: | ---: |
| Fixed MapLight local | 0.786 ± 0.004 | 0.720 ± 0.002 | 0.881 ± 0.001 |
| Fixed MapLight published | 0.783 ± 0.002 | 0.723 ± 0.003 | 0.881 ± 0.001 |
| Fixed + GIN local | 0.858 ± 0.001 | 0.791 ± 0.002 | 0.916 ± 0.000 |
| Fixed + GIN published | 0.859 ± 0.001 | 0.790 ± 0.001 | 0.916 ± 0.000 |

Every local seed-metric mean is within 0.003 AUPRC of its dated anchor. The
separately labeled local mean-probability AUPRC values are 0.7875, 0.7225, and
0.8827 for fixed MapLight and 0.8599, 0.7937, and 0.9165 for fixed plus GIN.
The maximum is 0.9165, so the mandatory 0.95 forensic gate did not trigger.
The scorecard manifest is `7ce51526...`.

Phase 0.5 had already observed this TDC public test. These results are therefore
bounded public-benchmark confirmations, not blind external validation. No
public-test-driven repair, feature choice, hyperparameter change, diagnostic
metric, or third-family prediction was performed.
...
The public comparator stage adds two byte-identical label-free prediction roots.
Each independently performs 30 fits and retains six family-task prediction
artifacts; only the canonical attempt consumes the six fixed/GIN task slots.
The bounded scorer traverses 37,550 canonical measurement rows, interprets
7,512 public labels, and performs 36 primary AUPRC calls. It performs zero
diagnostic metrics, new fits, new predictions, third-family operations, or
challenge assumptions. The preserved first scoring blocker contains zero
traversed rows, labels, and metrics.
...
explicitly unknown. The bounded public confirmation reproduces all six dated
anchors without a repair cycle. It does not authorize sequential public-test
tuning, a third TDC family, or a challenge model.
